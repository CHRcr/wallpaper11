...
抽取权重为：所有词基础权重 `1.0`；进阶词额外 `+2.0`，挑战词额外 `+4.0`；有词族、重要搭配或易混关系时再分别增加 `+0.3`、`+0.2`、`+0.2`。因此每个词都可出现，同时更适合清北班的 285 个重点词会明显提高频率。按当前词表总权重计算，单张词卡命中进阶或挑战词的概率约为 26.13%，每次两词同屏至少出现一个的概率约为 45.44%；最近 18 个学习组仍会被短期避开。
